...
*/
export async function calcAi(math_text) {
  const api_keys = [
  ...
  while (attempt < api_keys.length) {
    ...
    try {
      ...
      // await new Promise(res => setTimeout(res, 10)); // delay dikit
    }finally{
      inCollection("calc_ai_logs")
      .then(col => col.insertOne({math_text, result}))
      .then(inserted => logger.info(`request baru dipenuhi`))
      .catch(e => logger.error("error saat logging calc ai!"));
    }

  }

  return {
    error: "Layanan sedang sibuk! Perlu beberapa saat lagi hingga layanan pulih!"
  }
}
